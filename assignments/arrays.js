...
console.log([lastCar.car_make, lastCar.car_model].join(" "));
...
let carModelsMapped = inventory.map(a => a.car_model).sort();

console.log(carModelsMapped)
...
const carYearsMapped = inventory.map(a => a.car_year);
...
const oldCarsFiltered = inventory.filter(car => car.car_year < 2000)

console.log(oldCarsFiltered.length)

console.log(oldCarsFiltered)

const oldCarsReduced = inventory.reduce((years, car) => {
...
const BMWandAudiFiltered = inventory.filter(car => car.car_make === "BMW" || car.car_make === "Audi")

console.log(JSON.stringify(BMWandAudiFiltered))
